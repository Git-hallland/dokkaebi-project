...
export default async function CommunityWritePage() {
  ...
  const [session, siteSettings] = await Promise.all([headers().then((value) => auth.api.getSession({ headers: value })), getSiteSettings()]);
  if (!session) {
    if (!siteSettings.guideWriteEnabled) forbidden();
