...
export default async function CommunityPage({ searchParams }: Props) {
  ...
  const [params, session, siteSettings] = await Promise.all([searchParams, headers().then((value) => auth.api.getSession({ headers: value })), getSiteSettings()]);
  ...
  const now = new Date();
  const writingAllowed = canCreateGuide(session?.user.role, siteSettings.guideWriteEnabled);
  return <div className={styles.page}>
    <header className={styles.header}><div><p>COMMUNITY</p><h1>공략게시판</h1><span>이용자가 직접 작성한 공략과 짧은 팁을 공유합니다.</span></div><div className={styles.writeArea}>{writingAllowed ? (session ? <Link className={styles.write} href="/community/write">글쓰기</Link> : <Link className={styles.write} href="/profile">로그인하고 글쓰기</Link>) : <><span className={`${styles.write} ${styles.writeDisabled}`} aria-disabled="true">글쓰기</span><small>공략 작성은 정식 오픈 후 이용할 수 있습니다.</small></>}</div></header>
