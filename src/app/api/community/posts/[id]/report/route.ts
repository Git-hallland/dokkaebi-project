...
export async function POST(request: Request, { params }: Context) {
  ...
  try {
    ...
    const post = await prisma.guidePost.findUnique({ where: { id: postId }, select: { authorId: true, deletedAt: true } });
    assertReportableTarget(session.user.id, post);
    const report = await prisma.guideReport.create({
      data: { reporterId: session.user.id, postId, reason: input.reason, description: input.description },
      select: { id: true },
    });
    ...
    console.error("Community post report creation failed.");
