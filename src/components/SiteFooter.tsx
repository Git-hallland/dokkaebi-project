import Link from "next/link";

import { SupporterRanking, type PublicSupporter } from "./SupporterRanking";

import styles from "./SiteFooter.module.css";

export function SiteFooter({ adEnabled, stickyAdEnabled, supporters }: Readonly<{ adEnabled: boolean; stickyAdEnabled: boolean; supporters: readonly PublicSupporter[] }>) {
  return (
    <footer className={`${styles.footer} ${stickyAdEnabled ? "" : styles.withoutStickyAd}`}>
      <div className={styles.inner}>
        {adEnabled ? (
          <aside className={styles.adSlot} aria-label="하단 배너 광고 영역">
            <span>광고 배너 영역</span>
            <p>광고 플랫폼을 검토한 뒤 이 위치에 배너가 표시될 수 있습니다.</p>
          </aside>
        ) : null}

        <div className={styles.information}>
          <strong>도깨비의 세계 WIKI</strong>
          <p>
            본 사이트는 팬이 운영하는 비공식 정보 사이트이며, 카카오게임즈·슈퍼캣의
            공식 서비스가 아닙니다.
          </p>
          <p>
            ‘도깨비의세계’ 명칭, 상표 및 게임 관련 자료의 권리는 카카오게임즈·슈퍼캣 등
            각 권리자에게 있습니다.
          </p>
          <div className={styles.featureCards} aria-label="커뮤니티와 후원 바로가기">
            <a
              className={styles.featureCard}
              href="https://open.kakao.com/o/gESeLyBi"
              target="_blank"
              rel="noopener noreferrer"
              aria-label="도깨비의세계 오픈톡방 새 탭에서 열기"
            >
              <svg aria-hidden="true" focusable="false" viewBox="0 0 24 24">
                <path d="M4 5.5h16v11H9l-5 3v-14Z" />
                <path d="M8 10h8M8 13h5" />
              </svg>
              <span><strong>도깨비의세계 오픈톡방</strong><small>유저들과 정보를 공유해보세요</small></span>
            </a>
            <a
              className={styles.featureCard}
              href="https://toon.at/donate/hallland"
              target="_blank"
              rel="noopener noreferrer"
              aria-label="관리자에게 후원하기 새 탭에서 열기"
            >
              <svg aria-hidden="true" focusable="false" viewBox="0 0 24 24">
                <path d="M12 20.2 4.8 13A5 5 0 0 1 12 6.1 5 5 0 0 1 19.2 13L12 20.2Z" />
              </svg>
              <span><strong>관리자에게 후원하기</strong><small>위키 운영을 응원해주세요</small></span>
            </a>
            <SupporterRanking supporters={supporters} />
          </div>
          <div className={styles.policyArea}>
            <Link href="/editorial-policy">위키 운영 원칙</Link>
          </div>
          <small>© 2026 DokkaebiProject. 사이트 자체 제작 문서에 한함.</small>
        </div>
      </div>
    </footer>
  );
}
